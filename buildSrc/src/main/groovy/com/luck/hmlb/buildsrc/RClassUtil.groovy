...
class RClassUtil {
    ...
    static void collectRInfo(File file) {
        println("collectRInfo---file--->" + file.absolutePath)
        def isR = isRClass(file.absolutePath)
        println("isR--->" + isR)
        if (!isR) {
            return
        }
            ...
            MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
                ...
                methodVisitor = new MethodVisitor(Opcodes.ASM5, methodVisitor) {
                    ...
                    void visitFieldInsn(int opcode, String owner, String name1, String desc1) {
                        println("replaceRInfo---owner--->${owner}---name1--->${name1}")
    ...
    static boolean isRClass(String classFilePath) {
        classFilePath ==~ '''.*\\\\R\\$.*\\.class|.*\\\\R\\.class''' //如果是windows
//        classFilePath ==~ '''.*/R\\$.*\\.class|.*/R\\.class''' // 如果是mac
    }
    ...
    static boolean isRFileExceptStyleable(String classFilePath) {
        classFilePath ==~ '''.*\\\\R\\$(?!styleable).*?\\.class|.*\\\\R\\.class''' // 如果是windows
//        classFilePath ==~ '''.*/R\\$(?!styleable).*?\\.class|.*/R\\.class''' // 如果是mac
    }
    ...
    static String getFullClassName(String filePath) {
        println("getFullClassName--filePath--->" + filePath)
        String mode = "\\debug\\"
